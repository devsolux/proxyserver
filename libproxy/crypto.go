...
func CreateKeyPair() *[2]bytes.Buffer {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"The DevSolux Project"},
		},
		NotBefore: time.Now(),
	...
	if os.IsNotExist(err) {
		...
		if err != nil {
			if runtime.GOOS == "darwin" {
				...
				_, err = dlgs.Warning("Proxyserver", "Proxyserver needs you to install a root certificate authority (cert.pem).\nPlease double-click the certificate file to open it in Keychain Access and follow the installation and trust process.\n\nFor more information about this process and why it's required, please click the DevSolux icon in the status tray and select 'Help'.\n\nClick OK when you have installed the certificate and marked it as trusted.")
			}

			if runtime.GOOS == "windows" {
				_ = exec.Command("explorer.exe", GetOrCreateDataPath()+string(os.PathSeparator)+"cert.cer").Run()
				_, err = dlgs.Warning("Proxyserver", "Proxyserver needs you to install a root certificate authority (cert.cer).\nPlease install the certificate (opened) into the 'Trusted Root Certification Authorities' store for the Local Machine.\n\nFor more information about this process and why it's required, please click the DevSolux icon in the system tray and select 'Help'.\n\nClick OK when you have installed the certificate and marked it as trusted.")
			}
...
func GetOrCreateDataPath() string {
	...
	// If the data directory stat fails because the directory does not exist,
