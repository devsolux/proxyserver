...
func proxyHandler(response http.ResponseWriter, request *http.Request) {
	...
	if request.Header.Get("Origin") == "" || !isAllowedOrigin(request.Header.Get("Origin")) {
		...
		// If it is not an allowed origin, redirect back to devsolux.com.
		response.Header().Add("Location", "https://devsolux.com/")
	...
	proxyRequest.Header.Set("Via", "Proxyserver/1.1")
	...
	if len(strings.TrimSpace(proxyRequest.Header.Get("User-Agent"))) < 1 {
		...
		proxyRequest.Header.Set("User-Agent", "Proxyserver/1.1")
	}
